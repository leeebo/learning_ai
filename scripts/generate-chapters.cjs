...
<body>
  ...
  <main class="wrap day-layout">
    ...
    <article class="content" data-day="${day.n}">
      ...
      <section><h2>核心概念</h2><ul>${day.concept.map(item => `<li>${escapeHtml(item)}</li>`).join('')}</ul></section>
      ...
      <section><h2>预习</h2><p>${day.next ? `下一天：<a href="day${String(day.n + 1).padStart(2, '0')}.html">Day ${day.n + 1} · ${escapeHtml(day.next)}</a>` : '整理一页项目架构图，记录模型、内存、延迟、功耗和异常降级策略。'}</p></section>
